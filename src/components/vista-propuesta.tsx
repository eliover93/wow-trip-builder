...
import { formatoMoneda, totalPresupuesto, type Actividad, type Dia, type Viaje } from "@/lib/trip";
...
/** Propuesta que ve el cliente final: portada, itinerario, presupuesto y pie de marca. */
export function VistaPropuesta({
  viaje,
  agencia,
  mostrarEnlaceBackoffice = false,
}: {
  viaje: Viaje;
  agencia?: AgenciaPropuesta | null;
  mostrarEnlaceBackoffice?: boolean;
}) {
    ...
    <main>
      <section ref={heroRef} className="relative h-[92vh] overflow-hidden">
        ...
        >
          ...
          >
            {agencia?.nombre || viaje.agencia} · para {viaje.cliente}
          </motion.p>
          <motion.h1
          ...
          <div className="mt-12 grid gap-8 lg:grid-cols-[1.4fr_1fr]">
            ...
            >
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                ...
                persona · {viaje.viajeros} viajeros
              </p>
      ...
      <div>
        ...
        <ol className="mt-7 space-y-1">
          {dia.actividades.map((a, i) => (
            <FilaActividad key={`${a.titulo}-${i}`} actividad={a} retardo={i * 0.08} />
          ))}
        </ol>
      </div>
    </motion.article>
...
function FilaActividad({ actividad, retardo }: { actividad: Actividad; retardo: number }) {
  const Icono = iconos[actividad.tipo] ?? Sparkles;
  return (
    <motion.li
